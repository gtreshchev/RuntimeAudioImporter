...
void UStreamingSoundWave::PopulateAudioDataFromDecodedInfo(FDecodedAudioStruct&& DecodedAudioInfo)
{
	...
	if (OnPopulateAudioDataNative.IsBound() || OnPopulateAudioData.IsBound())
	{
		...
		AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr<UStreamingSoundWave>(this), PCMData = MoveTemp(PCMData)]() mutable
		{
			if (WeakThis.IsValid())
			{
	...
	{
		AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr<UStreamingSoundWave>(this)]()
		{
			if (WeakThis.IsValid())
			{
